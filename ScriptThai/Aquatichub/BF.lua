...
Auto:Toggle("Auto Electric Claw",nil,function(vu)
		Electricclow = vu
		if vu then
			local args = {
				[1] = "BuyElectro"
...
Auto:Toggle("ออโต้เอนมะ",false,function(v)
...
Auto:Toggle("จุดไฟทูชิตะ",false,function(v)
    if not Three_World then
...
Auto:Toggle("ออโต้เรนโบวหาคิ",false,function(a)
    if Three_World then
...
Dungeon:Line()
   ...
   Dungeon:Toggle("Kill Auraเปิดตอนลงดัน", false, function(vu)
    Killaura = vu
end)
...
Setting:Toggle("ออโต้ซื้อสีอาคิ",true,function(d)
_G.AUTOHAKI = d
end)
spawn(function()
    while wait(.1) do
...
Cr:Button("Copy Link Discord",function()
    setclipboard("ไม่รู้ ไม่ต้องถามแปลไทย")
end)
